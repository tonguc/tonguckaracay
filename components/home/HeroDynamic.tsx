...
/**
 * Bölüm 1 — Hero.
 * H1 + alt başlık + istatistikler tek büyük panelde birleşti (ayrı yüzen
 * kutular değil, tek görsel ağırlık). Servis etiketleri (GEO, reklam
 * yönetimi dahil) başlığın altında küçük pill'ler olarak — H1'i şişirmeden
 * kapsamı gösteriyor. Son yazı, panelin dışında ikincil bir şerit olarak.
 */
export default function HeroDynamic({ locale }: Props) {
  ...
  return (
    <section className="relative overflow-hidden pt-32 pb-16 md:pt-44 md:pb-28">
      {/* Arka plan gradient blob'ları */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-24 left-1/4 h-72 w-72 md:h-[28rem] md:w-[28rem] rounded-full bg-accent-500/10 blur-3xl animate-pulse" style={{ animationDuration: "5s" }} />
        <div className="absolute bottom-0 right-1/4 h-72 w-72 md:h-[28rem] md:w-[28rem] rounded-full bg-indigo-500/10 blur-3xl animate-pulse" style={{ animationDuration: "6s", animationDelay: "1s" }} />
      </div>

      <div className="container-custom relative z-10">
        <div className="mx-auto max-w-4xl">
          {/* Büyük hero paneli — H1 + alt başlık + servis etiketleri + CTA + istatistik, tek görsel bütün */}
          <div className="card flex flex-col items-center px-6 py-12 text-center md:px-14 md:py-16">
            ...
            {/* Servis etiketleri — H1'e sığmayan kapsamı burada gösteriyoruz */}
            ...
            {/* İstatistikler — panelin doğal devamı, kendi kutusu yok */}
          ...
          {/* Son yazı — panelin dışında, ikincil bir şerit */}
          {latest && (
            <Link
              href={postPath!}
              className="card group mt-6 flex w-full items-center gap-4 p-3 text-left transition-all duration-300 hover:-translate-y-0.5 md:p-4"
            >
              {latest.image && (
