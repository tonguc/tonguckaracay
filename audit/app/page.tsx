...
// ── Design tokens (inline styles for no-CSS-module setup) ────────────────────
const C = {
...
function ScoreRing({ score, size = 120, stroke = 10, color = C.accent }: {
  ...
  useEffect(() => {
    let v = 0
    const t = setInterval(() => {
      v += 2; if (v >= score) { setVal(score); clearInterval(t) } else setVal(v)
    }, 16)
  ...
  const grade = val >= 90 ? 'A' : val >= 70 ? 'B' : val >= 50 ? 'C' : 'D'
    ...
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
        ...
        <span style={{ fontSize: size > 100 ? 28 : 18, fontWeight: 800, color: C.text,
          fontFamily: 'var(--font-mono)', lineHeight: 1 }}>{val}</span>
        <span style={{ fontSize: size > 100 ? 12 : 10, color, fontWeight: 700,
          fontFamily: 'var(--font-mono)' }}>Grade {grade}</span>
      </div>
    </div>
  )
}
      ...
      <nav style={{ padding: '16px 12px', flex: 1 }}>
        {nav.map(item => (
          <div key={item.id} onClick={() => setPage(item.id)} style={{
            ...
            color: page === item.id ? C.accent : C.muted, transition: 'all 0.15s',
          }}>
      ...
      <div style={{ padding: '16px 24px', borderTop: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: '50%',
            ...
            fontSize: 13, fontWeight: 700, color: '#fff' }}>T</div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: C.text }}>Tonguc K.</div>
            <div style={{ fontSize: 10, color: C.muted }}>Agency Plan</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{ padding: '32px 36px', flex: 1, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between',
        ...
        <div>
          <div style={{ fontSize: 11, color: C.muted, fontWeight: 600,
            ...
            {auditUrl} · {new Date().toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' })}
          </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button style={{ background: 'transparent', border: `1px solid ${C.border}`,
            borderRadius: 8, padding: '9px 18px', color: C.muted, fontSize: 13 }}>
            ↓ Export PDF
          </button>
          <button onClick={() => setPage('audit')} style={{ background: C.accent, border: 'none',
            borderRadius: 8, padding: '9px 18px', color: '#fff', fontSize: 13, fontWeight: 600 }}>
            + New Audit
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 20, marginBottom: 20 }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`,
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: C.muted, fontWeight: 600,
              textTransform: 'uppercase', letterSpacing: '0.08em' }}>Weighted Score</div>
            ...
            <div key={cat.id} style={{ background: C.surface, border: `1px solid ${C.border}`,
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ background: `${cat.color}22`, border: `1px solid ${cat.color}44`,
                  ...
                  color: cat.color, fontFamily: 'var(--font-mono)' }}>{cat.grade}</div>
              </div>
              <div style={{ marginTop: 12, background: C.border, borderRadius: 4, height: 4 }}>
                <div style={{ height: 4, borderRadius: 4, background: cat.color,
                  width: `${cat.score}%`, transition: 'width 0.8s ease' }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`,
        ...
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${C.border}`,
          ...
          <div style={{ fontSize: 11, color: C.muted }}>{issues.length} issues found</div>
        </div>
        {issues.slice(0, 10).map((issue, i) => (
          <div key={issue.id} style={{ padding: '13px 24px',
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: SEV_COLOR[issue.priority] ?? C.muted,
              textTransform: 'uppercase', letterSpacing: '0.06em', width: 60, textAlign: 'right' }}>
              {issue.priority}
            </div>
          </div>
        ))}
...
function AuditView({ onComplete }: {
  ...
  const runAI = async () => {
    ...
    const t = setInterval(() => {
      ...
      if (p >= 90) { clearInterval(t); setProgress(90) }
      else setProgress(p)
    }, 120)
  ...
  if (step === 'scanning') return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
      ...
      <div style={{ background: C.border, borderRadius: 4, height: 4, overflow: 'hidden' }}>
        <div style={{ height: 4, background: C.accent, borderRadius: 4,
  ...
  if (step === 'manual') {
    ...
    const filled = allItems.filter(i => results[i.id]).length
      ...
      <div style={{ padding: '32px 36px', flex: 1, overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between',
          ...
          <button onClick={finishManual} style={{ background: C.green, border: 'none',
            borderRadius: 8, padding: '10px 20px', color: '#fff', fontWeight: 600, fontSize: 13 }}>
            Complete Audit →
          </button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {AUDIT_CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setActiveCat(c.id)} style={{
              ...
              fontSize: 12, fontWeight: activeCat === c.id ? 700 : 400 }}>
              {c.icon} {c.label}
            </button>
          ))}
        </div>
            ...
            {sec.items.map((item, i) => (
              <div key={item.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 14,
                ...
                <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 10, fontWeight: 700, color: SEV_COLOR[item.priority] }}>
                  {item.num}
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  {(['Pass', 'Partial', 'Fail', 'N/A'] as const).map(s => (
                    <button key={s} onClick={() => setResults(r => ({ ...r, [item.id]: s }))} style={{
                      ...
                      border: `1px solid ${results[item.id] === s ? (
                        s === 'Pass' ? C.green : s === 'Fail' ? C.red :
                        s === 'Partial' ? C.amber : C.muted
                      ) : C.border}`,
                      background: results[item.id] === s ? (
                        s === 'Pass' ? `${C.green}22` : s === 'Fail' ? `${C.red}22` :
                        s === 'Partial' ? `${C.amber}22` : `${C.muted}22`
                      ) : 'transparent',
                      color: results[item.id] === s ? (
                        s === 'Pass' ? C.green : s === 'Fail' ? C.red :
                        s === 'Partial' ? C.amber : C.muted
                      ) : C.muted,
                    }}>{s}</button>
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: 32, fontWeight: 800, color: C.text,
        letterSpacing: '-0.03em', marginBottom: 10 }}>Enter a URL to audit</h2>
      <p style={{ color: C.muted, fontSize: 14, lineHeight: 1.7, marginBottom: 32 }}>
        Claude AI will analyze <strong style={{ color: C.text }}>190 UX + SEO signals</strong> and
        generate a full scored report.
      </p>

      {/* Mode toggle */}
      <div style={{ display: 'flex', background: C.surface, borderRadius: 10,
        ...
        {(['ai', 'manual'] as const).map(m => (
          ...
          }}>{m === 'ai' ? '⚡ AI Auto-Audit' : '✍️ Manual Audit'}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
        ...
        <button onClick={mode === 'ai' ? runAI : () => setStep('manual')} style={{
          ...
          padding: '14px 24px', color: '#fff', fontWeight: 700, fontSize: 14, whiteSpace: 'nowrap' }}>
          {mode === 'ai' ? 'Run Audit →' : 'Start →'}
        </button>
      </div>
      {error && <div style={{ fontSize: 12, color: C.red, marginTop: 8 }}>{error}</div>}
      <div style={{ display: 'flex', gap: 20, justifyContent: 'center', marginTop: 16 }}>
        {['190 checks', 'AI-powered', 'PDF report', 'White label'].map(t => (
    ...
    <div style={{ padding: '32px 36px', flex: 1, overflowY: 'auto' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, marginTop: 6 }}>
          Customize branding on all client-facing PDF reports.</p>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, maxWidth: 680 }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24 }}>
          ...
          {[
            { label: 'Agency Name', value: name, onChange: setName, type: 'text', placeholder: 'Your Agency Name' },
          ].map(f => (
            <div key={f.label} style={{ marginBottom: 16 }}>
              ...
              <input value={f.value} onChange={e => f.onChange(e.target.value)}
                placeholder={f.placeholder} style={{ width: '100%', background: C.bg,
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 13, color: C.muted, fontFamily: 'var(--font-mono)' }}>{color}</span>
            </div>
          </div>
          ...
          <div style={{ background: C.bg, borderRadius: 10, padding: 20, border: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {['Technical','On-Page','UX','CRO'].map((l, i) => (
                <div key={l} style={{ flex: 1, background: C.surface, borderRadius: 6, padding: '8px', textAlign: 'center' }}>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, color: C.text }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        {page === 'audit'      && <div style={{ flex: 1, display: 'flex', overflowY: 'auto' }}>
                                    <AuditView onComplete={handleAuditComplete} /></div>}
        {page === 'whitelabel' && <WhiteLabelView />}
        {['reports','settings'].includes(page) && (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 48 }}>🚧</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: C.text }}>{page} — coming soon</div>
          </div>
        )}
      </div>
    </div>
